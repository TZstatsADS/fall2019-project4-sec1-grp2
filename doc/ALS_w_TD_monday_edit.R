...
data$times <- anytime(data$timestamp)
unique.timestamps <- unique(data$timestamp)
#time.key <- as.data.frame(unique.timestamps) %>%
#  group_by()
datat <- data %>% 
  arrange(timestamp) #orders time.key by ascending timestamp (assumes ALS function splits into bins by timestamp below)

datat$year <- format(datat$times,"%Y")
bins <- split(datat, datat$year) #if ALS code below doesn't automatically split it into groups. 23 bins this way
##come back to this - outputs each bin as list??
...
als.function <- function(){
  ...
  #initialize storage for bin time data
  time_item_bin_means <- matrix()
  for (t in 1:num.bins) {
    
    #take mean of item throughout bin
    #want to group by movie. have mean rating for each movie in bin
...
#average rating in bin
bi.bin.t <- mean(bin) - bi
